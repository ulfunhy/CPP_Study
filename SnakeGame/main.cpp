...
int main(void)
{
	...
	int snake_dir = DIR_DOWN;		// 뱀이 이도아는 방향
	...
	apple.setPosition(rand() % (apple_x * BLOCK_SIZE), rand() % (apple_y * BLOCK_SIZE));
	...
	while (window.isOpen())
	{
		...
		snake.setPosition(snake_x * BLOCK_SIZE, snake_y * BLOCK_SIZE);		// 픽셀 좌표
